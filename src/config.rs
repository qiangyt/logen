...
pub struct CliArgs {
    ...
    #[clap(value_parser, required = false, default_value = "<example>")]
...
static EXAMPLE_CONFIG_FILE: &str = "logen.config.example.yaml";
...
pub fn with_cli_args() -> Result<Option<AppDef>> {
    ...
    if args.example {
        let a = Asset::get(EXAMPLE_CONFIG_FILE).unwrap();
